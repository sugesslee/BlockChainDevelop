...
import com.nwnu.blockchain.utils.TrustSDKUtil;
...
public class InstructionService {
	...
	public boolean checkKeyPair(InstructionBody instructionBody) throws TrustSDKException {
		return TrustSDKUtil.checkPairKey(instructionBody.getPrivateKey(), instructionBody.getPublicKey());
	}
	...
	public Instruction build(InstructionBody instructionBody) throws Exception {
		...
		//设置签名，供其他人验证
		instruction.setSign(TrustSDKUtil.signString(instructionBody.getPrivateKey(), buildStr));
		//设置hash，防止篡改
		instruction.setHash(CryptoUtil.getSHA256(buildStr));
	...
	public boolean checkSign(Instruction instruction) throws TrustSDKException {
		String buildStr = getSignString(instruction);
		return TrustSDKUtil.verifyString(instruction.getPublicKey(), buildStr, instruction.getSign());
	}

	public boolean checkHash(Instruction instruction) {
		String buildStr = getSignString(instruction);
		return CryptoUtil.getSHA256(buildStr).equals(instruction.getHash());
	}
}
