package com.nwnu.blockchain.p2p.check.local;
...
public class DbBlockChecker implements BlockChecker {
	...
	public int checkHash(Block block) {
		...
		if (localLast != null && StrUtil
				.equals(localLast.getBlockHash(), block.getBlockHeader().getHashPreviousBlock())) {
			return 0;
		}
		return -1;
	}
	...
	private boolean checkBlockHashSign(Block block) {
		...
		String hash = CryptoUtil.getSHA256(block.getBlockHeader().toString() + block.getBlockBody().toString());
