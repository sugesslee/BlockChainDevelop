 ...
 *
 * @author red
 * @version 1.0.0 2019/11/18 9:54 AM
 * @since 1.0.0
 */
public class ECDSAAlgorithm {
	...
	public static String generatePrivateKey() {
		...
		while (privateKeyCheck.compareTo(BigInteger.ZERO) == 0 || privateKeyCheck
				.compareTo(Constants.MAXPRIVATEKEY) > 0) {
			secureRandom.nextBytes(privateKeyAttempt);
...
//		System.arraycopy(checkSumBytes, 0, rawAddr, versionHashBytes.length, 4);
	...
	public static boolean verify(String srcStr, String sign, String pubKey) throws Exception {
		byte[] hash256 = BaseAlgorithm.encode("SHA-256", srcStr.getBytes(StandardCharsets.UTF_8));
		...
		ECPublicKeyParameters params = new ECPublicKeyParameters(CURVE.getCurve().decodePoint(pub.getEncoded(true)),
				CURVE);
